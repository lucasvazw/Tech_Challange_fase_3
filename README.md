...
│   ├── data/
│   │   └── power_bi/
│   │       ├── dim_estados_ibge.csv
│   │       ├── gold_metricas_modelos.csv
│   │       └── gold_predicoes_risco.csv
│   │
│   ├── images/
│   │   ├── curva_roc.png
│   │   ├── matriz_confusao.png
...
```text
510 valores nulos em id_escola
```
...
O campo `id_escola` é tratado como variável categórica durante a preparação.
...
```text
ML/images/matriz_confusao.png
ML/images/curva_roc.png
```

As métricas comparativas dos modelos são exportadas para:

```text
ML/data/power_bi/gold_metricas_modelos.csv
```
...
```text
in_alfabetizado_real
in_alfabetizado_pred
score_prob
cluster_risco
```

O `score_prob` representa a probabilidade calculada pelo modelo.
...
```text
ML/data/power_bi/gold_predicoes_risco.csv
```

e também podem ser gravados no Databricks em:

```text
workspace.default.gold_predicoes_risco_ml
```
...
- `in_presenca_lp` não apresenta variação na Gold utilizada;
...
```text
ML/models/modelo_alfabetizacao.joblib
```
